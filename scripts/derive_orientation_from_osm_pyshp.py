...
COAST_DIR = Path("data/osm_coastlines")  # <-- agora é diretório, não ficheiro fixo

# distância ponto-segmento com correção simples da lon
...
def derive_orientation():
    ...
    shp_candidates = list(COAST_DIR.glob("*.shp"))
    if not shp_candidates:
        raise FileNotFoundError("Não encontrei nenhum .shp em data/osm_coastlines. Faz download e extrai o ZIP para aí.")
    r = shapefile.Reader(str(shp_candidates[0]))
    shapes = r.shapes()
    ...
    def in_pt_bbox(lon, lat):
        return (
            ...
            (-17.6 <= lon <= -15.8 and 32.2 <= lat <= 33.2)
        )
